...
let ChatService = class ChatService {
    ...
    async getMessages(userId, page = 1, limit = 50, targetUserId) {
        ...
        const messagesWithAvatars = messages.map(m => ({
            ...
            sender: {
                ...
                profile: {
                    photoUrl: m.sender.profile?.avatar ? `/uploads/profile/${m.sender.profile.avatar}` : '/uploads/profile/default.png',
                },
            },
            receiver: {
                id: m.receiver.id,
                ...
                profile: {
                    photoUrl: m.receiver.profile?.avatar ? `/uploads/profile/${m.receiver.profile.avatar}` : '/uploads/profile/default.png',
                },
            },
        }));
    ...
    async markMessagesAsRead(userId, targetUserId) {
        try {
            ...
            let whereClause = { receiverId: userId, isRead: false };
            if (targetUserId) {
                whereClause = {
                    ...whereClause,
                    OR: [
